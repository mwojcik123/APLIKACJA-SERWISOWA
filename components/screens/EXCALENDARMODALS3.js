...
import {
  ...
  updateEvent,
  getEvent,
  getNextCalendarDaily,
  getPrevCalendarDaily,
...
const clientCountOverview = events => {
  if (events == undefined) {
    return 0;
  }
  var count = 0;
...
class Calendary extends Component {
  ...
  componentDidMount() {
    ...
    setTimeout(
      () => this.reff2.current.scrollTo({x: width, animated: false}),
      0,
    );
  ...
  handleScrollForDays = e => {
    // e.preventDefault();
    const offset2 = e.nativeEvent.contentOffset.x;
    const contentHeight2 = e.nativeEvent.contentSize.width;
    const scrollViewHeight2 = width;
    console.log('offset2');
    console.log(offset2);
    console.log('contentHeight2');
    console.log(contentHeight2);
    console.log('contentHeight2 - width - 1');
    console.log(contentHeight2 - width - 1);

    if (offset2 <= 0) {
      // console.log('ugibugi');

      this.props.getPrevCalendarDaily();

      this.reff2.current.scrollTo({x: width, animated: false});
    }
    if (offset2 >= contentHeight2 - scrollViewHeight2 - 1) {
      console.log('dipa123');
      e.preventDefault();
      const wielkosc = this.props.days_format.length;
      console.log(wielkosc);
      this.props.getNextCalendarDaily();
      if (wielkosc >= 5) {
        console.log('width - po IF');
        console.log(width);
        this.reff2.current.scrollTo({x: width, animated: false});
      }
    }
  ...
  toggleView = () => {
    Animated.timing(this.animation, {
      toValue: this.state.isListView ? 0 : 1,
      duration: 500,
  ...
  WithoutSobotaus = () => {
    console.log(this.props.member_calendar);
    data = {
      isSobotaus: false,
    };
    this.props.updateMembersSobotaus(data, this.props.member_calendar.id);
  ...
  render() {
    const translateX = this.animation.interpolate({
      inputRange: [0, 1],
      outputRange: [0, -width], // Aktualny widok przesuwa się do góry
    });

    const translateXNew = this.animation.interpolate({
      inputRange: [0, 1],
      outputRange: [width, 0], // Nowy widok wchodzi od dołu
    });
    ...
    const noBussy = data => (
      <TouchableOpacity
        key={data.id}
        style={{elevation: 4, backgroundColor: '#fff'}}
        onPress={() => {
          this.props.getEvent(data.id);
          this.props.navigation.navigate('ShowEventCalendar');
        }}>
        <View
          style={[
            ...
            data.client
              ? {
                  borderTopWidth: 6,
                  borderTopColor: data.client.color,
                }
              : null,
          ]}>
    ...
    const events = this.props.format_day.events;
    return (
      <View style={{width: '100%', height: '100%'}}>
          ...
          <View style={[styles.headerAction, {alignItems: 'flex-end'}]}>
            <View style={{flexDirection: 'row'}}>
              {/* <TouchableOpacity
                ...
                <FeatherIcon name="grid" size={26} color="#185aca" />
        ...
        <View
          ...
          <Animated.View
            ...
            <View
              ...
              <ScrollView
                ...
                horizontal
                pagingEnabled
                extraData={this.state.isListView}
              ...
              >
                ...
                {weeks.map((item, index) => {
                  ...
                  const clientCountSob = clientCount(
                    days[item[6].format('YYYY-MM-DD')],
                  );

                  const clientOverviewCountPon = clientCountOverview(
                    days[item[1].format('YYYY-MM-DD')],
                  );
                  const clientOverviewCountWto = clientCountOverview(
                    days[item[2].format('YYYY-MM-DD')],
                  );
                  const clientOverviewCountSro = clientCountOverview(
                    days[item[3].format('YYYY-MM-DD')],
                  );
                  const clientOverviewCountCzw = clientCountOverview(
                    days[item[4].format('YYYY-MM-DD')],
                  );
                  const clientOverviewCountPnt = clientCountOverview(
                    days[item[5].format('YYYY-MM-DD')],
                  );
                  const clientOverviewCountSob = clientCountOverview(
                    days[item[6].format('YYYY-MM-DD')],
                  );
                  const sobotau = days[item[6].format('YYYY-MM-DD')];
                  const clientOverviewCount =
                    clientOverviewCountPon +
                    clientOverviewCountWto +
                    clientOverviewCountSro +
                    clientOverviewCountCzw +
                    clientOverviewCountPnt +
                    clientOverviewCountSob;
                  // console.log(item);
                  // console.log(item[0].format('YYYY-MM-DD'));
                  // console.log(clientCountPon);
                  // console.log(clientCountPon);
                      ...
                      <View style={styles.calendarHeder}>
                        <View style={{flex: 1, width: 40}}>
                          <Text
                            ...
                            {!member_calendar.isSobotaus && sobotau > 0 ? (
                              <FeatherIcon
                                name="alert-triangle"
                                color="#2c2c2c"
                                size={12}
              ...
              {/* <RBSheet
            customStyles={{container: styles.magicSheet}}
            height={300}
            openDuration={250}
            ref={this.sheet}>
            <View style={styles.fancyHeader}>
              <Text style={styles.fancyHeaderTitle}>Uwaga!</Text>
            </View>

            <View style={styles.snazzyBody}>
              <Text style={styles.snazzyBodyText}>
                czy chcesza aby kalendarz tego pracownika rozszerzyć o sobote??
              </Text>
              {member_calendar.isSobotaus ? (
                <TouchableOpacity onPress={() => this.WithoutSobotaus()}>
                  <View style={styles.funkyButton}>
                    <Text style={styles.funkyButtonText}>Bez Soboty</Text>
                  </View>
                </TouchableOpacity>
              ) : (
                <TouchableOpacity onPress={() => this.WithSobotaus()}>
                  <View style={styles.funkyButton}>
                    <Text style={styles.funkyButtonText}>Dodaj Soboty</Text>
                  </View>
                </TouchableOpacity>
              )}
              <View style={styles.snazzyBodyGap} />

              <TouchableOpacity
                onPress={() => {
                  this.sheet.current.close();
                }}>
                <View style={styles.chillButton}>
                  <Text style={styles.chillButtonText}>Anuluj</Text>
                </View>
              </TouchableOpacity>
            </View>
          </RBSheet> */}
          ...
          <Animated.View
            style={{
              position: 'absolute',
              transform: [{translateX: translateXNew}],
            }}>
            <View
              style={{
                width: '100%',
              ...
              <ScrollView
                ...
                showsHorizontalScrollIndicator={false}
                ref={this.reff2}
                onScroll={this.handleScrollForDays}
                scrollEventThrottle={0} // Częstotliwość zdarzenia onScroll
                // scrollTo={}
              >
                {console.log(this.state.today)}
                {days_format.map((item, index) => {
                  // const sobotau = days[item[6].format('YYYY-MM-DD')];
                  // const clientOverviewCount =
                  //   clientOverviewCountPon +
                  //   clientOverviewCountWto +
                                ...
                                {eventItem.bussy ? (
                                  <View style={styles.statsItem}>
                                    ...
                                    <Text
                                      ...
                                      -
                                      {eventItem.godzina_wizyty2.substring(
                                        0,
                                        2,
                                      )}
                                    </Text>
...
const styles = StyleSheet.create({
  ...
  cell6: {width: (width - 25) / 5},
  cell5: {width: (width - 25) / 5},
  containert: {
    paddingTop: 5,
    flexDirection: 'row',
    // height: '100%',
